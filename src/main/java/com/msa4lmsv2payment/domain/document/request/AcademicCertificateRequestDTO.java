...
public record AcademicCertificateRequestDTO(
        @Schema(description = "증명서 종류(ENROLLMENT: 재학증명서, GRADUATION: 졸업증명서, GRADE: 성적증명서)", example = "ENROLLMENT")
        @NotNull(message = "documentType은 필수입니다.")
        DocumentType documentType
) {
    @Schema(hidden = true)
    @AssertTrue(message = "documentType은 ENROLLMENT, GRADUATION 또는 GRADE만 가능합니다.")
    public boolean isStudentDocumentType() {
        return documentType == null
                || documentType == DocumentType.ENROLLMENT
                || documentType == DocumentType.GRADUATION
                || documentType == DocumentType.GRADE;
    }
}
