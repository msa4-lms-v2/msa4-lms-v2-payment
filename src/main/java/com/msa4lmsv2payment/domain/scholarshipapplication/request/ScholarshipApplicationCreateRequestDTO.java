...
public record ScholarshipApplicationCreateRequestDTO(
        ...
        @Schema(description = "신청 금액(원 단위 정수)", example = "1000000", requiredMode = Schema.RequiredMode.REQUIRED)
        @NotNull(message = "신청 금액은 필수입니다.")
        @DecimalMin(value = "1", message = "신청 금액은 0보다 커야 합니다.")
        @Digits(integer = 12, fraction = 0, message = "신청 금액은 원 단위 정수만 허용합니다.") BigDecimal requestedAmount,
