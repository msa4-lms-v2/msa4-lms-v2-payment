...
class AdminDashboardQueryRepositoryTest {
    ...
    @BeforeAll static void start() {
        ...
        jdbc.execute("create table scholarship_applications(id bigint primary key, tuition_bill_id bigint, student_id bigint, status varchar(30), created_at datetime)");
    ...
    @BeforeEach void reset() {
        for (String table : new String[]{"tuition_bills","installment_plans","scholarship_applications","student_snapshots","scholarships","payments","refunds"}) jdbc.update("delete from " + table);
    ...
    @Test void pendingSummaryAndTasksUseCurrentSemesterAndIncludeScholarships() {
        jdbc.update("insert into tuition_bills values (1,55,20,1000),(2,66,19,1000)");
        ...
        jdbc.update("insert into scholarship_applications values (101,1,55,'REQUESTED','2024-12-31'),(102,1,55,'APPROVED','2024-12-30'),(103,2,66,'REQUESTED','2024-01-01')");
        assertThat(queries.summary(20)).isEqualTo(new AdminDashboardResponse.Summary(35,1));
        assertThat(queries.tasks(20)).hasSize(30);
        assertThat(queries.tasks(20).getFirst().type()).isEqualTo("SCHOLARSHIP");
        assertThat(queries.tasks(20).getFirst().tuitionBillId()).isEqualTo(1L);
        assertThat(queries.tasks(20).getFirst().requesterName()).isEqualTo("학생 #55");
    }
}
