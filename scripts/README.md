...
별도 터미널에서 `http://127.0.0.1:18089/control/status`가 ready인지 확인한 뒤 `Invoke-RestMethod -Method Post http://127.0.0.1:18089/control/run`으로 실행한다. 정상 흐름의 실제 3분 재확인과 재발급 계좌의 알림 유실 복구를 포함하므로 수분이 걸린다. 결과는 workspace `analytics/evidence/msa4-lms-v2/admission-e2e-20260915/result.json`에 저장된다.
